...
# ---------------------------------------------------------
# 1. CẤU HÌNH CỨNG PHÂN LẬP TUYỆT ĐỐI (BASE)
# ---------------------------------------------------------
PROJECT_ROOT = Path(__file__).resolve().parents[1]
DATA_DIR = PROJECT_ROOT / "data"
RESULTS_DIR = PROJECT_ROOT / "results"
CHECKPOINT_DIR = PROJECT_ROOT / "checkpoints" / "sst2_modernbert_base"
TEST_CSV_PATH = DATA_DIR / "glue-sst2" / "test.csv"

MODEL_NAME = "AnswerDotAI/ModernBERT-base"
MODEL_SLUG = MODEL_NAME.replace("/", "_")
RESULTS_FILE = RESULTS_DIR / f"sst2_modernbert_base_{MODEL_SLUG}.json"
...
def main():
    ...
    print("\n[3/5] Đang thiết lập cấu hình Trainer...")
    data_collator = DataCollatorWithPadding(tokenizer=tokenizer)

    # ĐÃ SỬA: Đồng bộ hóa toàn bộ các siêu tham số vàng từ hàm finetune của bài báo
    training_args = TrainingArguments(
        ...
        # BẬT TỐI ƯU HÓA ĐỘ CHÍNH XÁC BF16 NHƯ BÀI BÁO
        bf16=torch.cuda.is_available(),
        bf16_full_eval=torch.cuda.is_available(), # Kích hoạt đánh giá bằng bf16 thuần
        fp16=False,
        tf32=torch.cuda.is_available(),
        gradient_checkpointing=True,
        dataloader_num_workers=4,
        
        # ĐÃ CẬP NHẬT: Tham số Optimizer độc quyền cho ModernBERT chống kẹt Loss
        optim="adamw_torch",
        adam_beta1=0.9,
        adam_beta2=0.98,                         # Thay vì 0.999 mặc định cũ
        adam_epsilon=1e-6,                       # Thay vì 1e-8 mặc định cũ
    ...
    trainer = Trainer(
        ...
        processing_class=tokenizer,             # Đồng bộ với kiến trúc transformers mới
    ...
    # ---------------------------------------------------------
    if not is_trained:
        print("\n[4/5] Bắt đầu quá trình Huấn luyện (Fine-tuning)...")
        trainer.train()
    ...
    record = {
        ...
        "device": device,
        "optimization": "flash_attention_2_cache_ready",
    }
